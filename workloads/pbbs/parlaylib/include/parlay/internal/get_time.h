...
private: 
  ...
  void report(double time, std::string str) {
    ...
    if (name.compare("Parlay time") == 0) {
      std::cout << "start,end: " << time_to_double(last) - time << "," << time_to_double(last) << std::endl;
    }
  ...
  double time_to_double(time_t t) {
    return std::chrono::duration_cast<std::chrono::microseconds>(t.time_since_epoch()).count() / 1000000.0;
  }
public:
  ...
  void next(std::string str) {
    ...
    // if (on) report_start_end(get_time(), str);
  }
