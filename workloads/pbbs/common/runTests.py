...
def runSingle(runProgram, options, ifile, procs) :
  ...
  try:
    ...
    start_end_times = [(str[str.index(':')+2:]) for str in out.split('\n') if str.startswith("start,end: ")]
    start = [float(start_end_time.split(',')[0]) for start_end_time in start_end_times]
    end = [float(start_end_time.split(',')[1]) for start_end_time in start_end_times]
    return times, start, end
  except (ValueError,IndexError):
    raise NameError(comString+"\n"+out)

def geomean(a) :
  return statistics.geometric_mean(a)
  # r = 1.0
  # for x in a :
  #   r = r * x
  # return r**(1.0/len(a))

def runTest(runProgram, checkProgram, dataDir, test, rounds, start, procs, noOutput, keepData) :
    random.seed()
    ...
    cwd = os.getcwd()
    currentDir = cwd.split('/')[-1]
    upperDir = cwd[:len(cwd)-len(currentDir)]
    fullDataDir = dataDir.replace("../", upperDir)
    fullInputNames = " ".join(fullDataDir + "/" + name for name in inputFileNames)
    # If file exists, don't regenerate
    dataPreGen = all(os.path.exists(name) for name in fullInputNames)
    if len(dataDir)>0 and not(dataPreGen):
      print("cd " + dataDir + "; make " + shortInputNames)
      out = shellGetOutput("cd " + dataDir + "; make " + shortInputNames)
    runOptions = runOptions + " -r " + repr(rounds)
    runOptions = runOptions + " -s " + repr(start)
    if (noOutput == 0) :
      runOptions = runOptions + " -o " + outFile
    initial_time = datetime.datetime.now()
    times, start, end = runSingle(runProgram, runOptions, longInputNames, procs)
    final_time = datetime.datetime.now()
    start_time= datetime.datetime.fromtimestamp(start[0])
    end_time = datetime.datetime.fromtimestamp(end[-1])
    startup_time = (start_time - initial_time).total_seconds()
    shutdown_time = (final_time - end_time).total_seconds()
    if (noOutput == 0) :
    ...
    print(shortInputNames + outputStr + " : "
          ...
          + ", startup = " + str(startup_time) + ", shutdown = " + str(shutdown_time))
    return [weight,times]
    
def averageTime(times) :
    return sum(times)/len(times)
    
def timeAll(name, runProgram, checkProgram, dataDir, tests, rounds, start, procs, noOutput,
            addToDatabase, problem, keepData) :
  totalTime = 0
  totalWeight = 0
  try:
    results = [runTest(runProgram, checkProgram, dataDir, test, rounds, start, procs,
                       noOutput, keepData)
               for test in tests]
...
def getArgs() :
  ...
  start = int(getArg("-s", 0))
  keep = getOption("-k")
  return (noOutput, rounds, start, addToDatabase, processors, keep)

def timeAllArgs(runProgram, problem, checkProgram, dataDir, tests, keepInputData=False) :
  keepData = keepInputData
  (noOutput, rounds, start, addToDatabase, procs, keep) = getArgs()
  keep = keepInputData or keep
  name = os.path.basename(os.getcwd())
  timeAll(name, runProgram, checkProgram, dataDir, tests, rounds, start, procs, noOutput, addToDatabase, problem, keep)
