...
def plot_rel_matrix_filtered(matrix_file, out_file, metric, workloads, with_iso=False):
    ...
    ax = sns.heatmap(matrix, annot=True, annot_kws={'size': 13},\
                cmap='coolwarm', fmt='.0f', cbar_kws={'label': metric, 'pad': 0.01, 'format' : ticker.PercentFormatter(xmax=100, symbol="")})
    cb = ax.collections[0].colorbar
    cb.set_label(metric, fontsize=15)
    ...
    yticks = yticks + 0.5
    plt.xticks(rotation=35, ha='right', labels=labels, ticks=xticks, fontsize=13)
    plt.yticks(rotation=0, ha='right', labels=labels, ticks=yticks, fontsize=13)
    ...
    plt.gca().xaxis.set_label_coords(0.45, -0.19)
    # Move the color bar to the right

    # Tweak the margins
    plt.subplots_adjust(left=0.14, right=1.013, top=0.97, bottom=0.19)
...
if __name__ == "__main__":
    ...
    plot_rel_matrix_filtered(runtime_relative_change_matrix_file, f'{figures_dir}/2a_runtime_relative_change_matrix_new.pdf', 'Runtime Change (%)', plot_workload_list, with_iso=False)

    proportional_energy_relative_change_matrix_file = f'{data_dir}/proportional_energy_relative_change_matrix.csv'
    plot_rel_matrix_filtered(proportional_energy_relative_change_matrix_file, f'{figures_dir}/2b_proportional_energy_relative_change_matrix_new.pdf', 'Energy Change (%)', plot_workload_list, with_iso=False)